...
var require_dist_node5 = __commonJS({
  'node_modules/@octokit/request/dist-node/index.js'(exports) {
    ...
    function fetchWrapper(requestOptions) {
      ...
      let status
      let url
      const fetch2 = requestOptions.request && requestOptions.request.fetch ||
        nodeFetch
      return fetch2(
        requestOptions.url,
        Object.assign(
    ...
    var Collection = class extends Node.NodeBase {
      ...
      hasAllNullValues(allowScalar) {
        return this.items.every((node) => {
          ...
          const n = node.value
          return n == null ||
            allowScalar && identity.isScalar(n) && n.value == null &&
              !n.commentBefore && !n.comment && !n.tag
        })
      }
      /**
...
var require_stringifyString = __commonJS({
  'node_modules/yaml/dist/stringify/stringifyString.js'(exports) {
    ...
    function doubleQuotedString(value, ctx) {
      ...
      return implicitKey ? str : foldFlowLines.foldFlowLines(
        str,
        indent,
...
${indent}`,
      ...
      return ctx.implicitKey ? res : foldFlowLines.foldFlowLines(
        res,
        indent,
...
${indent}${body}`
    ...
    function plainString(item, ctx, onComment, onChompKeep) {
      ...
      if (
        !implicitKey && !inFlow && type !== Scalar.Scalar.PLAIN &&
        value.includes('\n')
      ) {
...
${indent}`,
      ...
      if (actualString) {
        const test = (tag) =>
          tag.default && tag.tag !== 'tag:yaml.org,2002:str' &&
          tag.test?.test(str)
...
var require_stringify = __commonJS({
  'node_modules/yaml/dist/stringify/stringify.js'(exports) {
    ...
    function stringifyProps(node, tagObj, { anchors: anchors$1, doc }) {
      ...
      const props = []
      const anchor = (identity.isScalar(node) || identity.isCollection(node)) &&
        node.anchor
  ...
  'node_modules/yaml/dist/stringify/stringifyPair.js'(exports) {
    ...
    function stringifyPair({ key, value }, ctx, onComment, onChompKeep) {
      ...
      let explicitKey = !simpleKeys &&
        (!key || keyComment && value == null && !ctx.inFlow ||
          identity.isCollection(key) || (identity.isScalar(key)
...
${indent}:`
      ...
      if (
        ...
        identity.isSeq(value) && !value.flow && !value.tag && !value.anchor
      ) {
...
var require_addPairToJSMap = __commonJS({
  'node_modules/yaml/dist/nodes/addPairToJSMap.js'(exports) {
    ...
    var isMergeKey = (key) =>
      key === MERGE_KEY ||
      identity.isScalar(key) && key.value === MERGE_KEY &&
        (!key.type || key.type === Scalar.Scalar.PLAIN)
    function mergeToJSMap(ctx, map, value) {
      const source = ctx && identity.isAlias(value)
...
${indent}${line}`
    ...
    function stringifyFlowCollection(
      ...
      for (let i = 0; i < items.length; ++i) {
        ...
        if (
          !reqNewline && (lines.length > linesAtValue || str2.includes('\n'))
        ) {
          reqNewline = true
        }
        lines.push(str2)
    ...
    var YAMLSet = class _YAMLSet extends YAMLMap.YAMLMap {
      ...
      get(key, keepPair) {
        ...
        return !keepPair && identity.isPair(pair)
          ? identity.isScalar(pair.key) ? pair.key.value : pair.key
          : pair
      }
      set(key, value) {
...
var require_timestamp = __commonJS({
  'node_modules/yaml/dist/schema/yaml-1.1/timestamp.js'(exports) {
    ...
    function stringifySexagesimal(node) {
      ...
      return sign +
        parts.map((n) => String(n).padStart(2, '0')).join(':').replace(
          /000000\d*$/,
          '',
        )
    }
    ...
    function getTags(customTags, schemaName) {
      ...
      if (!tags) {
        if (Array.isArray(customTags)) {
          ...
          const keys = Array.from(schemas.keys()).filter((key) =>
            key !== 'yaml11'
      ...
      return tags.map((tag) => {
        ...
        const keys = Object.keys(tagsByName).map((key) => JSON.stringify(key))
          .join(', ')
  ...
  'node_modules/yaml/dist/compose/util-map-includes.js'(exports) {
    ...
    function mapIncludes(ctx, items, search) {
      ...
      const isEqual = typeof uniqueKeys === 'function'
        ...
        : (a, b) =>
          a === b ||
          identity.isScalar(a) && identity.isScalar(b) && a.value === b.value &&
  ...
  'node_modules/yaml/dist/compose/resolve-flow-collection.js'(exports) {
    ...
    function resolveFlowCollection(
      ...
      const NodeClass = tag?.nodeClass ??
        (isMap ? YAMLMap.YAMLMap : YAMLSeq.YAMLSeq)
  ...
  'node_modules/yaml/dist/compose/compose-collection.js'(exports) {
    ...
    function composeCollection(CN, ctx, token, tagToken, onError) {
      ...
      const expType = token.type === 'block-map'
        ...
        : 'seq'
      if (
        !tagToken || !tagName || tagName === '!' ||
        tagName === YAMLMap.YAMLMap.tagName && expType === 'map' ||
        tagName === YAMLSeq.YAMLSeq.tagName && expType === 'seq' || !expType
      ) {
        return resolveCollection(CN, ctx, token, onError, tagName)
      }
      let tag = ctx.schema.tags.find((t) =>
        t.tag === tagName && t.collection === expType
      )
      if (!tag) {
      ...
      const res = tag.resolve?.(coll, (msg) =>
        onError(tagToken, 'TAG_RESOLVE_FAILED', msg), ctx.options) ?? coll
  ...
  'node_modules/yaml/dist/compose/compose-scalar.js'(exports) {
    ...
    function findScalarTagByTest(
      ...
      const tag = schema.tags.find((tag2) =>
        tag2.default && tag2.test?.test(value)
...
${comment}`
            ...
            identity.isCollection(dc) && !dc.flow && dc.items.length > 0
          ) {
            let it = dc.items[0]
            if (identity.isPair(it)) {
              it = it.key
    ...
    function resolveAsScalar(token, strict = true, onError) {
      if (token) {
        const _onError = (pos, code, message) => {
          const offset = typeof pos === 'number'
            ? pos
            : Array.isArray(pos)
            ? pos[0]
            : pos.offset
...
var require_cst = __commonJS({
  'node_modules/yaml/dist/parse/cst.js'(exports) {
    ...
    var isScalar = (token) =>
      !!token &&
      (token.type === 'scalar' || token.type === 'single-quoted-scalar' ||
        token.type === 'double-quoted-scalar' || token.type === 'block-scalar')
    ...
    var Lexer = class {
      ...
      *lex(source, incomplete = false) {
      ...
      continueScalar(offset) {
        ...
        if (ch === '-' || ch === '.') {
          ...
          if (
            (dt === '---' || dt === '...') && isEmpty(this.buffer[offset + 3])
          ) {
            return -1
          }
        }
      ...
      *parseStream() {
        ...
        if (line[0] === '%') {
          ...
          const n = (yield* this.pushCount(dirEnd)) +
      ...
      *parsePlainScalar() {
        ...
        while (ch = this.buffer[++i]) {
          if (ch === ':') {
            ...
            if (
              next === '#' || inFlow && invalidFlowScalarChars.includes(next)
            ) {
              break
            }
            if (ch === '\n') {
      ...
      *pushIndicators() {
        switch (this.charAt(0)) {
          ...
          case ':': {
            ...
            if (
              isEmpty(ch1) || inFlow && invalidFlowScalarChars.includes(ch1)
            ) {
              if (!inFlow) {
                this.indentNext = this.indentValue + 1
        ...
        if (this.charAt(1) === '<') {
          ...
          while (ch) {
            if (tagChars.includes(ch)) {
              ...
              ch === '%' && hexDigits.includes(this.buffer[i + 1]) &&
      ...
      *pop(error) {
        ...
        if (!token) {
          ...
          if (token.type === 'block-scalar') {
            ...
            token.type === 'flow-collection' && top.type === 'document'
          ) {
            token.indent = 0
          }
...
var require_public_api2 = __commonJS({
  'node_modules/yaml/dist/public-api.js'(exports) {
    ...
    function parseOptions(options) {
      ...
      const lineCounter$1 = options.lineCounter ||
        prettyErrors && new lineCounter.LineCounter() || null
    ...
    function parseDocument(source, options = {}) {
      ...
      let doc = null
      for (
        const _doc of composer$1.compose(
...
async function action() {
  const config = {
    ...
    src: (0, import_core.getInput)('src'),
  }
  ...
  if (config.src.startsWith('http://') || config.src.startsWith('https://')) {
    ...
    file = await (0, import_promises.readFile)(config.src, {
      encoding: 'utf-8',
    })
  }
